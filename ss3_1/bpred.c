	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    /* bimodal component */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred2bit, li, 0, 0, 0);

    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, li, l2size, sw, 0);
  ...
  case BPred2Level:
    pred->dirpred.twolev = 
      bpred_dir_create(class, li, l2size, sw, xor);

    break;

  case BPred2bit:
    pred->dirpred.bimod = 
      bpred_dir_create(class, li, 0, 0, 0);
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	fatal("shift register width, `%d', must be non-zero and positive",
      ...
      temp = calloc(1, sizeof(unsigned char));
  ...
  case BPred2bit:
    ...
    for (cnt = 0; cnt < l1size; cnt++)
      {
	pred_dir->config.bimod.table[cnt] = 0;//flipflop;
	flipflop = 3 - flipflop;
      }
  ...
  switch (pred_dir->class) {
	...
	set = 0;
	...
	else
	  {
        ...
        int mask = pow(2, pred_dir->config.two.shift_width) - 1;
        
	    for(int i = pred_dir->config.two.shift_width; i > 0; i--)
	    {
	    	
	    	if(((l2index >> i) & mask) == (l2index & mask))
	    	{
	    		set = 1;
	    		temp = ((l2index >> (i-1)) & 0x1) ? 2 : 0;
	    	}
	    }
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPred2Level || pred->class == BPredComb))
    {
      ...
      long int mask = (pow(2, pred->dirpred.twolev->config.two.shift_width*2) - 1);
